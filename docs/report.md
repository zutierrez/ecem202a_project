...
## Metrics of Success
The overall metric for project success will be power consumption which will be calculated given voltage measurements and current measurements in various test cases. As a pra indicator of project viability in real-world applications, the team will translate power consumption to energy savings (mAh) which will be beneficial for indicating how these approaches may influence battery-powered systems. This metric will be our primary unit for measurring energy savings since it is a boardly used metric for batteries to which future work can be compared. Analysis will compare power consumption for communication-free multi-camera set up to the proposed cross-camera communication system built from scratch. 
