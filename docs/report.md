...
Novel research has proposed methods for reducing power consumption of edge devices. One method proposed by J. Lim, J. Seo, and Y. Baek proposes an energy-efficient approach which reduces the frequency of image transmission to a shared cloud service. In this work, "CamThings: IoT Camera with Energy-Efficient Communication by Edge Computing based on Deep Learning", the authors lower frequency of transmission and propose a deep neural network to identify images of interest in order to achieve energy conservation. Power consumption via this novel approach achieved a 41% decrease compared to a baseline method. This approach is limited in that it only explores the reduction of power consumption when deploying the approach for a single device. Much more energy savings can be achieved if this approach were deployed for a multi-camera system.

Another novel research approach from H. B. Pasandi and T. Nadeem, presented in "CONVINCE: Collaborative Cross-Camera Video Analytics at the Edge", aims to take advantage of spatio-temporal correlations between cameras as well as knowledge sharing to reduce overall power consumption. By reducing the number of redundant images sent to a shared cloud service, the proposed approach aims to reduce the costly power consumption related to sending images to the cloud. This method was found to reduce transmission of redundant images by 75%; however, this approach still has the potential for further improvements. This work is limited in its analysis of energy savings associated with less frequency image transmission. Further analysis could still be done to quantify the benefits of the approach proposed by CONVINCE.

## Novelty & Rationale
Our approach is to combine the energy-efficient approaches proposed by CONVINCE and CamThings to create a novel dual-camera embedded system. To improve power consumption of edge-computing image sensors, we first aim to reduce the frequency of transmission by utilizing image capture scheduling as well as deploying an image classification model to only transmit events of interest. Second, we aim to reduce image transmission by preventing transmission of redundant events of interest that have been captured by both image sensors in our network. Improved power consumption demonstrated from the CONVINCE and CamThings methods provide a strong rationale for further analysis of multi-camera systems and expected improvements if the two approaches are combined in a multi-camera deep learning network for transmission of event images.
...
## Metrics of Success
The overall metric for project success will be power consumption which will be calculated given voltage measurements and current measurements in various test cases. As a practical indicator of project viability in real-world applications, the team will translate power consumption to energy savings (mAh) which will be beneficial for indicating how these approaches may influence battery-powered systems. This metric will be our primary unit for measuring energy savings since it is a broadly used metric for battery capacity. Analysis will compare power consumption for communication-free multi-camera set up to the proposed cross-camera communication system built from scratch. 
...
* We took the same current measurements without USB power and found that the average current draw was decreased by around 0.2 mA - again. We considered this amount to be negligible since it was less than 1% of the average current draw.
    
**Case 2**: Classification + BLE Communication Program
* In this program, the Arduino searches for a peripheral device to connect to over Bluetooth, and only classifies images once it has found and connected to another device. The general flow is as follows: search for a peripheral device, connect, delay, capture image and inference, disconnect. The cycle then repeats. The instantaneous current draw of the device for each of these stages was 13.3 mA, 10.3 mA, 28-29 mA, 31-32 mA, and 20-21 mA, respectively. 
...
* In this case, we measured the current consumption of the device when it transmitted the binary data of an image to the computer. We found that the idle, take image, and transmission of image stages had currents of about 25 mA, 29 mA and 27 mA, respectively. 
...
In summary, we found that the current consumption of the Arduino varied with different loads and different stages in the programs. The highest current was found when the Arduino was executing the Classification + BT program, specifically in the “capture image” stage at 31-32 mA. 
...
* The image taking portion of the program took around 2 seconds, classification/inferencing took around 330 ms, image transmission took around 320 ms, and each frame had a 2 second delay to lower capture frequency. Other time durations (such as bluetooth label communication) were negligible.
* Say the region of overlap in the camera’s perspectives is 50%, and the number of captured frames that have a redundant subject is also 50%. Then, the total time for each frame capture and processing is ~4.65 seconds (2+0.330+0.32+2). Now, we can estimate the total energy consumed in a single frame. 
	...
	* Total energy consumed = (2/3600 hours) * (27+2 mA) + (0.33/3600 hours) * (28+2 mA) + (0.32/3600 hours) * (27+2 mA) + (2/3600 hours) * (24+2 mA) = **35.9 mAh**
		* This is calculated using data from Case 1, which had the most consistent readings. In this case, image capture required 27 mA and image classification took 28 mA. In Case 3, it was measured that image transmission took 27 mA. In Case 2, we measured BLE increased power consumption by about 2 mA. This was also factored in the approximation by adding to each current measurement.
...
* The power consumption of Bluetooth can be estimated as follows:
	* Energy consumed by BLE per frame = 2 mA * (4.65/3600 hours) = 2.58 mAh
